...
static var _INSTANCE: DT_Interface

static func delete():
	_INSTANCE = null


static func get_instance() -> DT_Interface:
	
	if !_INSTANCE || DT_Plugin.get_instance().get_dev_reset_instance() == "true":
		_INSTANCE = DT_Interface.new()
